...
Die folgenden Pfade beziehen sich auf das **Monorepo msa-build** (Komponenten unter `backend/`). Nach einem **flat deploy** aus `msa-deploy` liegen `mdns-hub/` und `mdns-sat/` typischerweise direkt im Installations-Root ohne `backend/`-Präfix; die relativen Pfade innerhalb der Komponenten bleiben gleich.
...
Das Script:
- Installiert Python-Abhängigkeiten in gemeinsames `venv` unter `backend/venv`
...
- Kopiert `mdns-sat/sat_config.example.yaml` zu `mdns-sat/sat_config.yaml`
...
```
msa-build/
├── frontend/             # React/Vite-Quellcode (Build nach backend/mdns-hub/frontend via CI/Release)
├── backend/
│   ├── mdns-hub/         # Hub-Komponente
│   │   ├── main.py       # FastAPI-App, API-Endpoints (systemd: Port 8080)
│   │   ├── run_hub.sh    # optional: lokaler Uvicorn-Start auf Port 8000
│   │   ├── models.py     # Pydantic-Models
│   │   ├── templates/    # HTML-Templates (Legacy-UI)
│   │   ├── frontend/     # React-Build-Output (Release)
│   │   └── hub_config.yaml
│   │
│   ├── mdns-sat/         # Satellite-Komponente
│   │   ├── mdns_sat.py   # Haupt-Script, FastAPI (Port 8080)
│   │   ├── run_sat.sh    # optional: Start mit venv aus backend/venv
│   │   ├── mdns_utils.py
│   │   ├── mdns_worker.py
...
│   ├── install_msa.sh
│   ├── manage_services.py
│   ├── venv/             # nach Setup (gemeinsame venv)
│   └── README.md
└── ...
```
...
Voraussetzung: `backend/venv` existiert (z. B. `cd backend && sudo ./install_msa.sh`, Menüpunkt **0) Nur venv / Python-Requirements installieren oder aktualisieren**).

**Hub (empfohlene Wrapper):**
```bash
cd backend/mdns-hub
./run_hub.sh
```
Entspricht im Wesentlichen `../venv/bin/uvicorn main:app --host 0.0.0.0 --port 8000` — Port **8000**, damit er sich vom systemd-Start über `main.py` (**8080**) unterscheidet.

Alternativ manuell auf 8080 wie im Dienst:
```bash
cd backend/mdns-hub
../venv/bin/uvicorn main:app --host 0.0.0.0 --port 8080 --reload
```

**Satellite (empfohlene Wrapper):**
```bash
cd backend/mdns-sat
./run_sat.sh
```
...
Das Frontend liegt im Monorepo unter `frontend/`:
- React + TypeScript
- Vite als Build-Tool
- Tailwind CSS + shadcn/ui
- Build-Output wird im Release-Workflow nach `backend/mdns-hub/frontend/` (bzw. ins Deploy-Payload `mdns-hub/frontend/`) kopiert
...
- Prüfe ob Service auf reporting interface gesehen wurde
- Prüfe Satellite-UI: `http://satellite-ip:8080/ui` (Spalte "Hub")
